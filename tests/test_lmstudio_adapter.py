...
class TestTorkLMStudioClient:
    """Tests for TorkLMStudioClient."""
    ...
    def test_govern_input_messages(self):
        ...
        client = TorkLMStudioClient(tork=self.tork)
    ...
    def test_govern_complete(self):
        """Test that governance is applied to complete."""
        from tork_governance.adapters.lmstudio_adapter import TorkLMStudioClient
    ...
    def test_govern_embeddings(self):
        """Test that governance is applied to embeddings."""
        from tork_governance.adapters.lmstudio_adapter import TorkLMStudioClient
        ...
        assert "[SSN_REDACTED]" in call_args.kwargs.get("input", "")
        assert len(result["_tork_receipts"]) > 0
    ...
    def test_skip_governance_when_disabled(self):
        ...
        client = TorkLMStudioClient(tork=self.tork, govern_input=False, govern_output=False)
...
class TestLMStudioGoverned:
    """Tests for lmstudio_governed decorator."""

    def test_decorator_governs_messages(self):
        """Test that the decorator governs messages kwarg."""
        from tork_governance.adapters.lmstudio_adapter import lmstudio_governed

        tork = Tork()

        @lmstudio_governed(tork)
